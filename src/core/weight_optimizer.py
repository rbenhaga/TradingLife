...
class WeightOptimizer:
    ...
    def __init__(self, exchange: Optional[ExchangeConnector] = None, testnet: bool = False):
        """
        Initialise l'optimiseur
        
        Args:
            exchange: Connecteur d'exchange pour récupérer les données
            testnet: Indique si le mode testnet doit être utilisé
        """
    ...
    def _optimize_single_wrapper(self, symbol: str, start_date: datetime, 
        ...
        api_key = os.getenv("BINANCE_API_KEY")
        api_secret = os.getenv("BINANCE_API_SECRET")
        asyncio.run(self.exchange.connect(api_key=api_key, api_secret=api_secret))
        
        return asyncio.run(self.optimize(symbol, start_date, end_date, n_trials, initial_capital))
    ...
    def save_results(self, filepath: str = 'optimization_results.json'):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            json.dump([r.__dict__ for r in self.optimization_history], f, ensure_ascii=False, indent=2) 